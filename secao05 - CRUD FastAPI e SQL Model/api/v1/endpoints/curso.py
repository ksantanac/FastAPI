...
async def post_curso(curso: CursoModel, db: AsyncSession = Depends(get_session)):
    # Cria uma nova instância do modelo CursoModel com os dados recebidos no payload
    novo_curso = CursoModel(titulo=curso.titulo, aulas=curso.aulas, horas=curso.horas)
    
    # Adiciona o novo curso na sessão do banco de dados
    db.add(novo_curso)
    
    # Espera a conclusão da operação de commit para confirmar a transação no banco de dados
    await db.commit()
...
async def delete_curso(curso_id: int, db: AsyncSession = Depends(get_session)):
    async with db as session:
        ...
        if curso_del:
            ...
            # Por conta de um bug no FastAPI
